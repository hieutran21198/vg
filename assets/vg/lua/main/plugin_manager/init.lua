...
M.disable = function(plugins)
    if plugins ~= nil then
        for _, v in ipairs(plugins) do store = table.remove_key(store, v) end
    end
end

M.setup = function(app_runtime_dir)
    local gpacker = "https://github.com/wbthomason/packer.nvim"
    if vim.fn.empty(vim.fn.glob(app_runtime_dir ..
                                    "/site/pack/packer/start/packer.nvim")) > 0 then
        local cmd = "!git clone " .. gpacker .. " " .. app_runtime_dir ..
...
M.compile = function()
    ...
    packer.startup(function(use)
        ...
        for _, plug in pairs(store) do use(plug) end
    end)
end
